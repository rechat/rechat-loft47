...
export const BrokerageDealAccessRolesService = {
  ...
  retrieveBrokerageDealAccessRole: async (
    brokerage_id: string,
    deal_access_role_id: string
  ) => {
    try {
      const data = await apiFetch(
  ...
  createBrokerageDealAccessRole: async (
    brokerage_id: string,
    deal_access_role: DealAccessRole
  ) => {
    try {
      const data = await apiFetch(
        `/loft47/brokerages/${brokerage_id}/deal_access_roles`,
  ...
  updateBrokerageDealAccessRole: async (
    brokerage_id: string,
    deal_access_role_id: string,
    deal_access_role: DealAccessRole
  ) => {
  ...
  deleteBrokerageDealAccessRole: async (
    brokerage_id: string,
    deal_access_role_id: string
  ) => {
    try {
      ...
      if (data.status === 204) {
        return {
          status: 204,
          message: 'Deleted successfully, no response body.'
        }
      }

      return {
        status: data.status,
        message: `Deleted failed, response body:${data.statusText}`
      }
    } catch (err: any) {
      return { status: err.status, error: err.body.error }
    }
